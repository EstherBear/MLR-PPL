...
def compute_pseudo_label_accuracy(meter, category_meter, pseudoLabel, groundTruth, margin=0.5):

    pseudoLabel, groundTruth = pseudoLabel.cpu().data.numpy(), groundTruth.cpu().data.numpy()
    classNum = groundTruth.shape[1]
    ...
    for classIdx in range(classNum):
        category_meter['precision'][classIdx].update(
            np.sum((pseudoLabel[:, classIdx] == 1).astype(np.float) * (groundTruth[:, classIdx] == 1).astype(np.float)), 
            np.sum(pseudoLabel[:, classIdx] == 1))
        category_meter['recall'][classIdx].update(
            np.sum((pseudoLabel[:, classIdx] == 1).astype(np.float) * (groundTruth[:, classIdx] == 1).astype(np.float)), 
            np.sum(groundTruth[:, classIdx] == 1))
    # Confuse Matrix
...
def an_compute_pseudo_label_accuracy(meter, category_meter, pseudoLabel, groundTruth, margin=0.5):
    # groundTruth {-1, 1}
    # pseudoLabel {0, 1}
    pseudoLabel, groundTruth = pseudoLabel.cpu().data.numpy(), groundTruth.cpu().data.numpy()
    classNum = groundTruth.shape[1]
...
def compute_inter_feature_distance(meter, feature, target):
    ...
    Shape of target : (BatchSize, classNum) {-1, 1}
    """
...
def an_compute_inter_feature_distance(meter, feature, target):
    """
    Shape of feature : (BatchSize, classNum, featureDim) 
    Shape of target : (BatchSize, classNum) {0, 1}
    """
